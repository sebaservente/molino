...
@section('admin')
    <div class="div__productos py-3 px-2">
        <h2 class="py-4 px-4 w-100 fontSize1 textBold8 text-uppercase text-center">Promos</h2>
...
{{--                                @if($promos->imagen != null && Storage::disk('public')->has('img/reserva/' . $promos->imagen))--}}
                                @if($promos->imagen != null && public_path('img/reserva' . '/' . $promos->imagen))
                                    <picture class="">
                                        <source media="(min-width: 751px)"
                                                srcset="{{ url('img/reserva/' . $promos->imagen) }}">
                                        <source media="(min-width: 380px)"
                                                srcset="{{ url('img/reserva/' . $promos->imagen) }}">
                                        <img src="{{ url('img/reserva/' . $promos->imagen) }}" class="w-100"
            ...
            <div class="modal fade" id="staticBackdrop2{{ $promos->producto_id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="staticBackdropLabel" aria-hidden="true">
                <div class="modal-dialog">
                    <div class="modal-content text-center">
                        <div class="modal-header">
                            <h2 class="modal-title textBold8" id="staticBackdropLabel">{{ $promos->titulo }}</h2>
                        ...
                        <div class="modal-footer">
                            <form action="{{ route('admin.delete', ['id' => $promos->producto_id]) }}" method="post">
                                @csrf
                ...
                <div class="div__adminProductos shadow mx-1 my-2">
                    <a href="#" class="text-decoration-none text-dark d-flex w-100 " data-bs-toggle="modal" data-bs-target="#staticBackdrop{{ $promos->producto_id }}">
                        ...
                        <div class="div__datosAdmin w-100 px-1">
                            <p class="datos__parrafoAdmin m-0 py-1">{{ $promos->titulo }}</p>
                          {{--  <p class="datos__descripcion">{{ $promos->descripcion }}</p>--}}
                        </div>
                        {{--<p class="fw-bold text-dark">$ <span> {{ $promos->precio }}</span></p>--}}
                    </div>
                    </a>
                </div>
            @endif
